...
int	check_parsing(t_pars *stock)
{
	if ((stock->number_of_coders < 1) || (
			stock->number_of_coders > 2147483647))
		return (1);
	else if ((stock->time_to_burnout < 1) || (
			stock->time_to_burnout > 2147483647))
		return (1);
	else if ((stock->time_to_compile < 1) || (
			stock->time_to_compile > 2147483647))
		return (1);
	else if ((stock->time_to_debug < 1) || (
			stock->time_to_debug > 2147483647))
		return (1);
	else if ((stock->time_to_refactor < 1) || (
			stock->time_to_refactor > 2147483647))
		return (1);
	else if ((stock->number_of_compiles_required < 1) || (
			stock->number_of_compiles_required > 2147483647))
		return (1);
	else if ((stock->dongle_cooldown < 1) || (
			stock->dongle_cooldown > 2147483647))
		return (1);
	else if ((strcmp(stock->scheduler, "fifo") != 0) && (
			strcmp(stock->scheduler, "edf") != 0))
		return (1);
	return (0);
}

void	parsing_input(t_pars *stock, char **str)
{
